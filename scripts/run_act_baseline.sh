#!/bin/bash
# Launch ACT baseline on StackCube-v1 pd_ee_delta_pos demos.
set -o pipefail
...
source "$ACT_DIR/configs/stackcube_pos_act_baseline.conf"
mkdir -p "$(dirname "$LOG_FILE")"
echo "[$(date +%H:%M:%S)] running ACT baseline -> $LOG_FILE"
