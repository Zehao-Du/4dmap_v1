#!/bin/bash
# DP + 4D map raw concat on PlugCharger-v1, 100 demos
set -o pipefail
...
RUN_NAME="plugcharger_dp_map4d_raw_100demos_seed1"
...
python train_rgbd.py \
  ...
  --num-demos 100 --control-mode pd_ee_delta_pose --sim-backend physx_cpu \
  ...
  --use-map4d --map4d-raw-concat --map4d-source maniskill_gt \
  --map4d-task-name PlugCharger-v1 --map4d-num-objects 2 \
