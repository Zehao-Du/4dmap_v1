#!/bin/bash
# ACT + raw concat + keyframe future loss + TCP pos, 100 demos
set -o pipefail
...
if [[ ! -f "${KEYFRAME_AUX_PATH}" ]]; then
  ...
  echo "Build it with: TASK_NAME=StackCube-v1 TCP_TARGET=${TCP_TARGET} bash scripts/data_collection/build_keyframe_aux_dataset.sh" >&2
  exit 1
fi
...
RUN_NAME="${RUN_NAME:-stackcube_pos_act_map4d_keyframe_tcp_${TCP_TARGET}_aux_100demos_seed1}"
