...
#
# Usage:
#   bash scripts/data_collection/DiTMap4D/build_training_dataset_from_scratch.sh <task> <demos> [resolution(default=224)] [generate_dino_feature(default=true)]
#
# Examples:
...
#     bash scripts/data_collection/DiTMap4D/build_training_dataset_from_scratch.sh plugcharger 1000 native false
#
# Outputs:
#   1) filtered ManiSkill RGB-D demo
#   2) in-place PPI-style fused point cloud: traj_*/obs/point_cloud/fused [T,P,6]
#   3) keyframe sidecar with node-pose and TCP targets
#   4) generate/validate unified DINO Semantic Field:
#      traj_*/obs/point_cloud/fused [T,P+N,6], traj_*/obs/dino_feature [T,P+N,D_sem]
...
if [[ $# -lt 2 || $# -gt 4 ]]; then
  echo "Usage: $0 <task> <demos> [resolution] [generate_dino_feature]" >&2
  ...
  echo "  generate_dino_feature: true|false, default true; false validates existing unified obs/dino_feature" >&2
  exit 1
fi
...
DATA_ROOT="${DATA_ROOT:-${POLICY_DIR}/dataset/maniskill}"
...
POINTCLOUD_SKIP_EXISTING="${POINTCLOUD_SKIP_EXISTING:-0}"
SEMANTIC_FEATURE_MODE="${SEMANTIC_FEATURE_MODE:-dinov3}"
SEMANTIC_FEATURE_PATH="${SEMANTIC_FEATURE_PATH:-obs/dino_feature}"
GENERATE_DINO_FEATURE="${GENERATE_DINO_FEATURE:-1}"
DINOV3_MODEL="${DINOV3_MODEL:-dinov3_vits16}"
DINOV3_WEIGHTS_PATH="${DINOV3_WEIGHTS_PATH:-/inspire/hdd/project/robot-dna/baojiachun-CZXS25130063/zehao/foundation_models/DINOv3/dinov3_vits16_pretrain_lvd1689m-08c60483.pth}"
DINOV3_THIRD_PARTY_DIR="${DINOV3_THIRD_PARTY_DIR:-${ROOT_DIR}/mappolicy/models/DINOv3/dinov3}"
...
ONLINE_DINO_SEMANTIC_FEATURE_DIM="${ONLINE_DINO_SEMANTIC_FEATURE_DIM:-}"
...
TARGET_FORMAT_POSE="map4d_dit_local_delta_relative_quaternion_v1"
TARGET_FORMAT_POS_GRIPPER="map4d_dit_local_delta_relative_quaternion_tcp_pos_gripper_v1"
...
case "${GENERATE_DINO_FEATURE,,}" in
  1|true|yes|y|on|generate|gen)
    GENERATE_DINO_FEATURE="1"
    ;;
  0|false|no|n|off|reuse|existing|skip)
    GENERATE_DINO_FEATURE="0"
    ;;
  *)
    echo "Invalid generate_dino_feature=${GENERATE_DINO_FEATURE}. Use true or false." >&2
    exit 1
    ;;
esac
...
if [[ "${GENERATE_DINO_FEATURE}" != "1" ]]; then
...
case "${RAW_TASK,,}" in
  stackcube|stackcube-v1)
    ...
    COLLECT_SCRIPT="${POLICY_DIR}/scripts/data_collection/dp_dataset/collect_stackcube.sh"
  ...
  plugcharger|plugcharger-v1)
    ...
    COLLECT_SCRIPT="${POLICY_DIR}/scripts/data_collection/dp_dataset/collect_plugcharger.sh"
...
pointcloud_cmd=(
  python "${POLICY_DIR}/scripts/data_collection/helpers/build_pointcloud_dataset.py"
...
if [[ "${POINTCLOUD_SKIP_EXISTING}" == "1" || "${POINTCLOUD_SKIP_EXISTING,,}" == "true" ]]; then
  echo "POINTCLOUD_SKIP_EXISTING is disabled; rebuild point clouds explicitly." >&2
  exit 1
fi
...
semantic_cmd=(
  python "${POLICY_DIR}/scripts/data_collection/helpers/build_point_semantic_features.py"
  --demo-path "${TRAIN_DEMO_PATH}"
  --mode "${SEMANTIC_FEATURE_MODE}"
  --map4d-sidecar-path "${KEYFRAME_SIDECAR_PATH}"
...
if [[ "${GENERATE_DINO_FEATURE}" != "1" ]]; then
  echo "  generate: false (validating existing unified precomputed DINO features)"
fi
if [[ "${SEMANTIC_FEATURE_MODE}" == "dinov3" ]]; then
...
context_cmd=(
  python "${POLICY_DIR}/scripts/data_collection/helpers/build_map4d_context_dataset.py"
  --demo-path "${TRAIN_DEMO_PATH}"
...
mkdir -p "$(dirname "${MANIFEST_PATH}")"
{
  ...
  printf 'GENERATE_DINO_FEATURE=%q\n' "${GENERATE_DINO_FEATURE}"
  if [[ "${GENERATE_DINO_FEATURE}" == "1" ]]; then
