#!/usr/bin/env bash
# Train PlugCharger Map4DDiT with the read-heavy HDF5 dataset staged in shared memory.
#
# Usage:
#   bash scripts/map4d_experiments/train_plugcharger_map4d_dit_shm.sh <num_demos> [hydra_overrides...]
#
# Optional environment variables:
#   SHM_ROOT=/dev/shm/4dmap_plugcharger_dit   Shared-memory staging directory.
#   FORCE_SHM_COPY=1                          Re-copy files even if staged copies exist.
#   CLEAN_SHM_ON_EXIT=1                       Remove staged files when training exits.
#   MANIFEST_PATH=/path/to/context.env        Source manifest to stage.
set -euo pipefail
...
DATASET_DIR="${DATASET_DIR:-${DATA_ROOT}/ManiSkill/PlugCharger-v1/motionplanning}"
FUTURE_HORIZON="${FUTURE_HORIZON:-4}"
SOURCE_MANIFEST="${MANIFEST_PATH:-${DATASET_DIR}/PlugCharger.rgb+depth.pd_ee_delta_pose.physx_cpu.filtered.map4d_dit_h${FUTURE_HORIZON}.context.env}"
SHM_ROOT="${SHM_ROOT:-/dev/shm/4dmap_plugcharger_dit}"
...
if [[ "${TASK_OVERRIDE}" != "task=plugcharger_map4d_dit" ]]; then
  echo "Expected PlugCharger manifest TASK_OVERRIDE=task=plugcharger_map4d_dit, got ${TASK_OVERRIDE}" >&2
  exit 1
fi
...
export TASK_KEY="plugcharger"
export MANIFEST_PATH="${SHM_MANIFEST_PATH}"
export ADDITION_INFO="${ADDITION_INFO:-plugcharger_${NUM_DEMOS}demos_${RESOLUTION_TAG}_4d_action}"
