...
def _load_keyframe_aux_tensors(keyframe_aux_path, *, num_traj=None):
    import h5py
            ...
            aux_trajs.append(
                {
                    "future_keyframe_indices": torch.as_tensor(
                        group["future_keyframe_indices"][()]
                    ).long(),
                    "future_keyframe_object_targets": torch.as_tensor(
                        group["future_keyframe_object_targets"][()]
                    ).float(),
                    "future_keyframe_tcp_pose": torch.as_tensor(
                        group["future_keyframe_tcp_pose"][()]
                    ).float(),
                }
            )
    return aux_trajs
...
class Args:
    ...
    """load torchvision pretrained backbone weights"""
...
class SmallDemoDataset_ACTPolicy(Dataset):
    def __init__(self, data_path, num_queries, num_traj, include_depth=True,
                 ...
                 map4d_keyframe_aux_loss=False, map4d_keyframe_aux_path=None):
        if data_path[-4:] == '.pkl':
        ...
        if self.map4d_keyframe_aux_loss:
            ...
            print("Loading keyframe aux tensors...")
            self.keyframe_aux_trajs = _load_keyframe_aux_tensors(
                map4d_keyframe_aux_path, num_traj=num_traj
            )
            print(f"Loaded {len(self.keyframe_aux_trajs)} keyframe aux trajectories.")
        ...
        if 'delta' in args.control_mode or args.control_mode == 'base_pd_joint_vel_arm_pd_joint_vel':
            self.pad_action_arm = torch.zeros((trajectories['actions'][0].shape[1]-1,))

        self.slices = []
        self.num_traj = len(trajectories['actions'])
        if self.keyframe_aux_trajs is not None and len(self.keyframe_aux_trajs) != self.num_traj:
            raise ValueError(
                f"Loaded {len(self.keyframe_aux_trajs)} keyframe aux trajectories for {self.num_traj} demos."
            )
        for traj_idx in range(self.num_traj):
            ...
            if self.keyframe_aux_trajs is not None:
                ...
                if aux_len != episode_len + 1:
                    raise ValueError(
                        f"Trajectory {traj_idx} keyframe aux length {aux_len} does not match actions length {episode_len} + 1."
                    )
    ...
    def __getitem__(self, index):
        ...
        if action_len < self.num_queries:
            if self.delta_control or args.control_mode == 'base_pd_joint_vel_arm_pd_joint_vel':
        ...
        if self.use_map4d:
            ...
            if self.keyframe_aux_trajs is not None:
                ...
                obs['future_keyframe_object_targets'] = aux_traj[
                    'future_keyframe_object_targets'
                ][ts]
                obs['future_keyframe_tcp_pose'] = aux_traj['future_keyframe_tcp_pose'][ts]
...
def _build_train_only_env(dataset):
    obs0 = dataset.trajectories['observations'][0]
    spaces_dict = {
        'state': spaces.Box(
            -float('inf'),
            float('inf'),
            shape=obs0['state'].shape[1:],
            dtype=np.float32,
        ),
        'rgb': spaces.Box(
            0,
            255,
            shape=obs0['rgb'].shape[1:],
            dtype=np.uint8,
        ),
    }
    if 'depth' in obs0:
...
class Agent(nn.Module):
    def __init__(self, env, args):
        ...
        if self.map4d_aux_loss or self.map4d_keyframe_aux_loss:
            ...
            self.future_pred_head = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(args.hidden_dim, future_target_dim),
            )
    ...
    def _policy_obs(obs):
        ...
        return {k: v for k, v in obs.items() if k not in aux_keys}

    def _aux_readout(self, encoder_memory):
        ...
        readout, _ = self.aux_cross_attn(query, memory_kv, memory_kv)
        return self.aux_norm(readout.squeeze(1))
    ...
    def compute_loss(self, obs, action_seq):
        ...
        elif self.map4d_keyframe_aux_loss and 'future_keyframe_object_targets' in obs:
            object_targets = obs['future_keyframe_object_targets'].flatten(start_dim=1)
            tcp_targets = obs['future_keyframe_tcp_pose'].flatten(start_dim=1)
            gt_target = torch.cat([object_targets, tcp_targets], dim=-1)
            readout = self._aux_readout(encoder_memory)
            pred_future = self.future_pred_head(readout)
            aux_loss = F.l1_loss(pred_future, gt_target)
            loss_dict['keyframe_aux_loss'] = aux_loss
            loss_dict['loss'] = loss_dict['loss'] + aux_loss * self.map4d_aux_weight
...
if __name__ == "__main__":
    ...
    if args.no_eval:
        envs = _build_train_only_env(dataset)
    ...
    for cur_iter, data_batch in enumerate(train_dataloader):
        ...
        # Evaluation
        if not args.no_eval and cur_iter > 0 and cur_iter % args.eval_freq == 0:
            last_tick = time.time()
            ema.copy_to(ema_agent.parameters())
        ...
        if cur_iter % args.log_freq == 0:
            ...
            if 'keyframe_aux_loss' in loss_dict:
                loss_str += f", keyframe_aux_loss: {loss_dict['keyframe_aux_loss'].item():.6f}"
            print(loss_str)
            ...
            if 'keyframe_aux_loss' in loss_dict:
                writer.add_scalar("losses/keyframe_aux_loss", loss_dict['keyframe_aux_loss'].item(), cur_iter)
