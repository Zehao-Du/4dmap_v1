...
class DiTMap4d(BasePolicy):
    def __init__(self, 
            ...
            action_type='single_arm_ee_pose',
        ...
        action_dim=None
        if action_type == 'single_arm_ee_pose':
            action_dim=8
        elif action_type == "single_arm_ee_pos":
            action_dim=4
        elif action_type == "dual_arm_ee_pose":
            action_dim=16
        if (action_dim==None):
            raise RuntimeError(f"Illegal action_type")
        if action_type != "single_arm_ee_pose":
            raise NotImplementedError("DiTMap4d currently supports only single_arm_ee_pose")
        
        # observation encoder
        obs_encoder = ObservationEncoder(
            state_shape=action_dim,
            out_channel=encoder_output_dim,
            dim_dino_feature=dino_feature_dim,
        ...
        self.rotation_6d_noise_scheduler = DDPMScheduler(
            num_train_timesteps=noise_scheduler_cfg.num_train_timesteps,
            beta_schedule="squaredcos_cap_v2",
            prediction_type=noise_scheduler_cfg.prediction_type
        )
        self.rotation_quat_noise_scheduler = DDPMScheduler(
            num_train_timesteps=noise_scheduler_cfg.num_train_timesteps,
            beta_schedule="squaredcos_cap_v2",
            prediction_type=noise_scheduler_cfg.prediction_type
        )
        ...
        # model
        model = Map4DDiT(
            action_horizon=horizon_action,
            keyframe_horizon=horizon_keyframe,
    ...
    def forward(self, batch):
        ...
        gt_openess = trajectory[..., 7:8]
        tcp_pos = self.position_noise_scheduler.add_noise(
            trajectory[..., :3], noise[..., :3], timesteps
        )
        tcp_rot = normalize_quaternion(
            self.rotation_quat_noise_scheduler.add_noise(
                trajectory[..., 3:7], noise[..., 3:7], timesteps
            )
        )
        noisy_trajectory = torch.cat((tcp_pos, tcp_rot, gt_openess), -1)
        ...
        for layer_pred_left in pred_left:
            ...
            if torch.numel(gt_openess) > 0:
                ...
                loss_left += 30*F.l1_loss(openess_left, gt_openess, reduction='mean')
            total_loss = total_loss + loss_left
        loss_dict = {
                'action_loss': loss_left.item(),
            }
