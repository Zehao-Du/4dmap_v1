...
def _maps4d_metadata_dir():
    return Path(__file__).resolve().parents[1] / "representation" / "maps4d"
...
class RoboGraphormer(nn.Module):
    ...
    def __init__(
        ...
        if num_nodes is None and map_name is not None:
            ...
            num_nodes = MAP_NODE_VOCAB[map_name]
        ...
        self.node_encoder = RoboNodeEncoder(
            ...
            semantic_vocab_size=len(SEMANTIC_NODE_VOCAB),
        )
    ...
    def forward(self, map_input, return_loss=False):
        ...
        data = _coerce_graph_data(map_input, complete_graph=self.complete_graph, device=device)
