...
class GeometricTransformerEncoder(nn.Module):
    """Encode 4D map sequences into per-step features using Transformer.

    Drop-in replacement for GeometricEncoder: same inputs, same outputs.
    Replaces the per-object GRU with a Transformer that jointly models
    spatial (inter-object) and temporal (inter-frame) relationships.
    """
    ...
    def __init__(
        ...
        # --- Spatial: node & relation encoding (same as GRU version) ---
        ...
        # --- Project node_dim → temporal_dim for transformer input ---
        self.input_proj = nn.Linear(node_dim, temporal_dim)

        # --- Positional encodings ---
        self.temporal_pos = nn.Embedding(max_seq_len, temporal_dim)
        self.object_pos = nn.Embedding(num_objects, temporal_dim)
        ...
        # --- Output projections (same interface as GRU version) ---
    ...
    def _encode_sequence_parts(self, rep_seq):
        ...
        node_feat = self.node_mlp(node_input)  # (B, T, N, node_dim)
        ...
        node_feat = node_feat + self.relation_proj(rel_agg)  # (B, T, N, node_dim)

        # --- Flatten to (B, T*N, temporal_dim) for transformer ---
        node_feat = self.input_proj(node_feat)  # (B, T, N, temporal_dim)

        # Add positional encodings: temporal + object identity
        t_idx = torch.arange(T, device=device)
        n_idx = torch.arange(N, device=device)
        t_pos = self.temporal_pos(t_idx)  # (T, temporal_dim)
        n_pos = self.object_pos(n_idx)    # (N, temporal_dim)
        # Broadcast: (T, 1, d) + (1, N, d) → (T, N, d)
        pos_enc = t_pos.unsqueeze(1) + n_pos.unsqueeze(0)  # (T, N, temporal_dim)
        node_feat = node_feat + pos_enc.unsqueeze(0)  # (B, T, N, temporal_dim)

        # Reshape to sequence: (B, T*N, temporal_dim)
        seq = node_feat.reshape(B, T * N, self.temporal_dim)

        # --- Transformer ---
        seq = self.layer_norm(seq)
        temporal_out = self.transformer(seq)  # (B, T*N, temporal_dim)

        # Reshape back: (B, T, N, temporal_dim)
        temporal_out = temporal_out.reshape(B, T, N, self.temporal_dim)

        # --- Output projections ---
        obj_feat = self.obj_proj(temporal_out)     # (B, T, N, feature_dim)
        scene_feat = obj_feat.mean(dim=2)          # (B, T, feature_dim)
        map_feature_seq = self.scene_proj(scene_feat)  # (B, T, feature_dim)

        return map_feature_seq, obj_feat, scene_feat, sizes, positions, rotations


# ---------------------------------------------------------------------------
# Shared utility functions (identical to geometric_encoder.py)
# ---------------------------------------------------------------------------
...
def _extract_map4d_sequence(seq):
    sizes_list, positions_list, rotations_list = [], [], []
    for item in seq:
